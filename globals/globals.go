...
var (
	...
	// DefaultAuthTokenMaxDuration sets the maximum token time to live globally for
	// all tokens generated by all auth methods
	DefaultAuthTokenMaxDuration = 7 * 24 * time.Hour

	// DefaultAuthTokenMaxStaleness sets the maximum time a token can live without
	// being used globally for all auth methods
	DefaultAuthTokenMaxStaleness = 24 * time.Hour
)
