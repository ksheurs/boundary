...
- `auth_token_max_duration` - Maximum time to live (TTL) for all auth tokens globally (pertains
...
- `auth_token_max_staleness` - Maximum time of inactivity for all auth tokens globally (pertains
to all tokens from all auth methods). Valid time units are anything specified by Golang's 
